...
<tbody v-for= "(items,i) in PaginationFiltre" :key ="items.idgroupeclient">
 ...
 <td>{{(++i)}}|{{items.idgroupeclient}}</td>
 <td> {{items.groupeclientCourt}}</td>
 <td>{{items.groupeclientLong}} </td>
 <td>
...
<label class ="blue-text text-darken-2" for="Initial">Réference</label> <input type="text" class="form-control" name="refgroupe" v-model="refgroupe" placeholder="Refernce du groupe"> 
<label for="Nom du groupe">Nom</label> <input type="text" class="form-control"  name="nomgroupe" v-model="nomgroupe" placeholder="nom du groupe"> 
...
export default {
  ...
  data () {
    return {
      titrepages: "Gestion des Groupes Clients",
        limitA: 0,
         LimitLignePage: 10,
        nombreligne:[
         {a : 10, value:"10"  },
...
ApiGroupeClient : 'parametrage/API_groupeclients/',
MessageErreurApi: '', 
valeur2 : '', 
...
axios.get(this.cheminUrl+this.ApiGroupeClient+'AfficheListeGroupeClient').then(response => {
this.info = this.copy_info = response.data 
this.nbrEnreg = Object.keys(this.info).length //récupère le nom d'enregistrement 
...
this.ErreurApi = "true"
this.MessageErreurApi = "Erreur de modification de la fiche / Merci de contacter le support"

}
,
ModifierFicheclient : function (i) {
let formData = new FormData();
formData.append('refgroupe',this.refgroupe)
formData.append('nomgroupe',this.nomgroupe)
...
this.titrepages = "Groupe Client | Nouveau"
...
this.titrepages = "Gesion des groupes clients | modification"
...
axios({
method:'POST',
url: this.cheminUrl+this.ApiGroupeClient+'ajoutGroupeClientdb', 
data:formData,
config: {headers: {'Content-Type': 'multipart/form-data'}}
}).then ((response) => {
